...
export default {
	...
	mounted() {
		let defaultOption = {
			...
			padding: [15, 0, 0, 0], //画布填充边距，顺序为上右下左，同css，但必须4位
			...
			yAxis: {
				...
				format: val => {
					let defaultSetting = { type: 'number', fixed: 1, name: '' };//fixed几位小数点
			...
			extra: {
				column: {
					//参考官网配置
					type: 'group',
					width: (this.cWidth * 0.35) / this.dataAs[this.labelKey].length
				}
			}
		};
